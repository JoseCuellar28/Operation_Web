...
class ModernDashboard {
        ...
        this.menuItems = [
            ...
            },
            {
                ...
                children: [
                    ...
                    { label: "Gestión de Vehículos", icon: "fa-truck", page: "gestion-vehiculos" },
                    { label: "Gestión de Materiales", icon: "fa-tools", page: "gestion-materiales" },
                    { label: "Gestión de Perfiles", icon: "fa-users-cog", page: "perfiles" },
    ...
    loadPage(pageName, pageTitle) {
        ...
        if (typeof UIComponents !== 'undefined') {
            if (pageName === 'colaboradores' && UIComponents.getColaboradoresTableContent) {
                ...
                return;
            } else if (pageName === 'perfiles' && UIComponents.getRolesGridContent) {
                // Roles handles its own container injection inside cargarRoles
                this.cargarRoles();
                return;
                ...
                return;
                return;
            } else if (pageName === 'gestion-vehiculos' && UIComponents.getVehiculosTableContent) {
                content = UIComponents.getVehiculosTableContent();
                mainContent.innerHTML = DOMPurify.sanitize(content, {
                    ADD_TAGS: ['style', 'tr', 'td', 'span', 'button', 'i', 'div', 'input', 'table', 'thead', 'tbody', 'th', 'form', 'label', 'select', 'option'],
                    ADD_ATTR: ['class', 'id', 'style', 'onclick', 'onkeyup', 'onchange', 'title', 'placeholder', 'type', 'value', 'name', 'data-state', 'required', 'step']
                });

                // Inject Modal Container
                if (!document.getElementById('modal-vehiculo-container')) {
                    const modalDiv = document.createElement('div');
                    modalDiv.id = 'modal-vehiculo-container';
                ...
                setTimeout(() => {
                    ...
                    const searchInput = document.getElementById('buscarVehiculos');
                    if (searchInput) {
    ...
    // --- VEHICLES MODULE LOGIC ---

    async cargarVehiculos() {
        const tbody = document.getElementById('vehiculos-tbody');
        if (!tbody) return;

        tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4">Cargando vehículos...</td></tr>';

        try {
            const res = await fetch(`${this.apiBaseUrl}/api/vehiculos`, {
                headers: this.getAuthHeaders()
            });
            if (res.ok) {
                const vehiculos = await res.json();
                if (vehiculos.length === 0) {
                    tbody.innerHTML = '<tr><td colspan="6" class="text-center p-8 text-muted-foreground">No hay vehículos registrados.</td></tr>';
                    return;
                }
                tbody.innerHTML = vehiculos.map(v => UIComponents.getVehiculoRow(v)).join('');
            } else {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4 text-red-500">Error al cargar vehículos.</td></tr>';
            }
        } catch (error) {
            console.error(error);
            tbody.innerHTML = '<tr><td colspan="6" class="text-center p-4 text-red-500">Error de conexión.</td></tr>';
        }
    }

    abrirModalVehiculo(id = null) {
        let content = '';
        let title = 'Nuevo Vehículo';

        if (id) {
            title = 'Editar Vehículo';
            // Fetch vehicle details first
            fetch(`${this.apiBaseUrl}/api/vehiculos/${id}`, { headers: this.getAuthHeaders() })
                .then(res => res.json())
                .then(data => {
                    content = UIComponents.getModalVehiculoContent(data);
                    this.renderModalVehiculo(title, content);
                })
                .catch(err => alert('Error al cargar datos del vehículo'));
        } else {
            content = UIComponents.getModalVehiculoContent({});
            this.renderModalVehiculo(title, content);
        }
    }

    renderModalVehiculo(title, content) {
        const container = document.getElementById('modal-vehiculo-container');
        container.innerHTML = UIComponents.getModalTemplate('modal-vehiculo', title, content, `
            <button onclick="window.dashboard.closeModal('modal-vehiculo')" class="px-4 py-2 border rounded-md hover:bg-accent">Cancelar</button>
            <button onclick="document.getElementById('form-vehiculo').dispatchEvent(new Event('submit'))" class="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90">Guardar</button>
        `);

        setTimeout(() => this.openModal('modal-vehiculo'), 50);
    }
    ...
    verVehiculo(id) {
        ...
        fetch(`${this.apiBaseUrl}/api/vehiculos/${id}`, { headers: this.getAuthHeaders() })
            .then(res => res.json())
            .then(data => {
                const content = UIComponents.getModalVehiculoContent(data);

                // Render with disabled inputs and NO Save button
                const container = document.getElementById('modal-vehiculo-container');
                container.innerHTML = UIComponents.getModalTemplate('modal-vehiculo', 'Detalle del Vehículo', content, `
                    <button onclick="window.dashboard.closeModal('modal-vehiculo')" class="px-4 py-2 bg-primary text-primary-foreground rounded-md hover:bg-primary/90">Cerrar</button>
                `);
                ...
                setTimeout(() => {
                    ...
                    this.openModal('modal-vehiculo');
                }, 50);
            })
            .catch(err => alert('Error al cargar datos del vehículo'));
    }

    async guardarVehiculo(mode, id) {
        const form = document.getElementById('form-vehiculo');
        const formData = new FormData(form);
        const data = Object.fromEntries(formData.entries());

        // Manual validation override for Moto/Volume
        if (data.TipoActivo === 'MOTO') data.MaxVolumen = 'BAJO';

        const url = mode === 'create' ? `${this.apiBaseUrl}/api/vehiculos` : `${this.apiBaseUrl}/api/vehiculos/${id}`;
        const method = mode === 'create' ? 'POST' : 'PUT';

        // If updating, include ID (Placa)
        if (mode === 'edit') data.Placa = id;
        ...
        try {
            ...
            if (res.ok) {
                this.closeModal('modal-vehiculo');
                this.cargarVehiculos();
                this.showNotification(`Vehículo ${mode === 'create' ? 'creado' : 'actualizado'} correctamente`, 'success');
            } else {
    ...
    async eliminarVehiculo(id) {
        ...
        try {
            const res = await fetch(`${this.apiBaseUrl}/api/vehiculos/${id}`, {
                method: 'DELETE',
                headers: this.getAuthHeaders()
            });
            ...
            if (res.ok) {
                this.cargarVehiculos();
                this.showNotification('Vehículo eliminado', 'success');
            } else {
    ...
    handleTipoActivoChange(select) {
        const capacitySelect = document.getElementById('vehiculo-capacidad');
        const motoInfo = document.getElementById('moto-info');

        if (select.value === 'MOTO') {
            capacitySelect.value = 'BAJO';
            capacitySelect.disabled = true;
            capacitySelect.classList.add('bg-gray-100');
            motoInfo.classList.remove('hidden');
        } else {
            capacitySelect.disabled = false;
            capacitySelect.classList.remove('bg-gray-100');
            motoInfo.classList.add('hidden');
        }
    }

    async checkDatabaseSetup() {
        if (!confirm('¿Desea intentar reparar la Base de Datos de Vehículos? Esto creará la tabla si no existe.')) return;

        try {
            const res = await fetch(`${this.apiBaseUrl}/api/vehiculos/setup-db`, {
                method: 'POST',
                headers: this.getAuthHeaders()
            });
            const text = await res.text();
    ...
    async cargarRoles() {
        try {
            ...
            if (container) {
                ...
                container.innerHTML = `
                    <div class="h-full flex flex-col animate-in fade-in zoom-in duration-300">
                        <!-- Header -->
                        <div class="border-b bg-white -mx-6 -mt-6 p-6 mb-6">
                            <div class="max-w-7xl mx-auto">
                                <h1 class="text-3xl font-bold text-gray-900">Gestión de Roles y Permisos</h1>
                                <p class="text-sm text-gray-500 mt-2">Administra roles, usuarios y permisos del sistema</p>
                            </div>
                        </div>
                        ...
                        <div class="border-b border-gray-200 mb-6">
                            <nav class="-mb-px flex space-x-8" aria-label="Tabs">
                                <button onclick="window.dashboard.switchRoleTab('roles')" id="tab-roles" class="border-blue-500 text-blue-600 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                                    Roles
                                </button>
                                <button onclick="window.dashboard.switchRoleTab('users')" id="tab-users" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                                    Asignar Usuarios
                                </button>
                                <button onclick="window.dashboard.switchRoleTab('permissions')" id="tab-permissions" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm transition-colors">
                                    Permisos
                                </button>
                            </nav>
                        </div>
                        ...
                        <!-- Tab Contents -->
                        <div id="content-roles" class="tab-content block">
                             <!-- Toolbar & Search -->
                             <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
                                <div class="relative flex-1 w-full sm:max-w-md">
                                    ...
                                    <input type="text" id="roles-search" class="block w-full pl-10 pr-3 py-2 border border-gray-300 rounded-lg leading-5 bg-white placeholder-gray-500 focus:outline-none focus:placeholder-gray-400 focus:ring-1 focus:ring-blue-500 focus:border-blue-500 sm:text-sm" placeholder="Buscar roles...">
                                </div>
                                <button onclick="window.dashboard.abrirModalCrearRol()" class="inline-flex items-center justify-center rounded-lg text-sm font-medium bg-blue-600 text-white hover:bg-blue-700 h-10 px-4 py-2 shadow-sm gap-2 transition-colors">
                                    <i class="fas fa-plus"></i> Crear Rol
                                </button>
                             </div>

                             <!-- Grid Container -->
                             <div id="roles-grid-container">
                         ...
                         <div id="content-users" class="tab-content hidden">
                            <div class="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                ...
                                <h3 class="text-lg font-medium text-gray-900">Asignación de Usuarios</h3>
                                <p class="text-sm text-gray-500 mt-1">Este módulo estará disponible próximamente.</p>
                            </div>
                        </div>
                         ...
                         <div id="content-permissions" class="tab-content hidden">
                            <div class="text-center py-12 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                ...
                                <h3 class="text-lg font-medium text-gray-900">Matriz de Permisos</h3>
                                <p class="text-sm text-gray-500 mt-1">La configuración granular de permisos se habilitará en la próxima actualización.</p>
                            </div>
                        </div>
                    </div>
                `;
                ...
                // Add Search Listener
                document.getElementById('roles-search')?.addEventListener('keyup', (e) => {
                    this.filtrarRoles(e.target.value);
                });
            }
            ...
            const grid = document.getElementById('roles-grid-container');
            if (grid) grid.innerHTML = `<div class="text-red-500 text-center py-4">Error al cargar roles: ${error.message}</div>`;
        }
    }

    filtrarRoles(searchTerm) {
        if (!searchTerm) {
            this.rolesFiltrados = [...this.roles];
        } else {
    ...
    renderizarRoles() {
        const grid = document.getElementById('roles-grid-container');
        if (grid && window.UIComponents) {
            grid.innerHTML = window.UIComponents.getRolesGridContent(this.rolesFiltrados);
        }
    }

    switchRoleTab(tabName) {
        // Update Content Visibility
        ['roles', 'users', 'permissions'].forEach(t => {
    ...
    abrirModalCrearRol() {
        this.openModalWithContent('Crear Nuevo Rol', window.UIComponents.getModalRolContent({}), 'modal-rol');
    }

    async editarRol(id) {
        try {
            const role = this.roles.find(r => r.id === id); // Optimistic fetch if available
            if (!role) throw new Error("Rol no encontrado en memoria");

            this.openModalWithContent('Configurar Rol', window.UIComponents.getModalRolContent(role), 'modal-rol');
        } catch (error) {
            console.error(error);
            alert('Error al abrir editor de rol');
        }
    }

    async guardarRol(mode, id) {
        ...
        const method = mode === 'edit' ? 'PUT' : 'POST';
        const url = mode === 'edit' ? `${this.apiBaseUrl}/api/roles/${id}` : `${this.apiBaseUrl}/api/roles`;
        ...
        try {
            ...
            if (!response.ok) {
                const err = await response.text();
                throw new Error(err || 'Error al guardar rol');
            }

            this.closeModal('modal-rol');
            await this.cargarRoles(); // Reload to refresh grid and counts
            alert(mode === 'edit' ? 'Rol actualizado' : 'Rol creado con éxito');
        } catch (error) {
            console.error(error);
            alert(`Error: ${error.message}`);
        }
    ...
    async cargarMateriales() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/api/materiales`, {
                headers: this.getAuthHeaders()
            });
            ...
            console.log("Materiales cargados (API):", data.length);

            this.materiales = data;
            // Initialize filtered list with full data
            this.materialesFiltrados = [...this.materiales];

            // Apply any existing filters (essential for stability)
            if (typeof this.filtrarMateriales === 'function') {
