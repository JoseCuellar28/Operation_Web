...
const UIComponents = {
            ...
            <div class="space-y-4">
                <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    <h2 class="text-2xl font-bold tracking-tight flex items-center gap-2">
                        <i class="fas fa-tools text-blue-600"></i> Gestión de Materiales
                    </h2>
                    <button onclick="abrirModalCrearMaterial()" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 bg-blue-600 text-white hover:bg-blue-700 h-10 px-4 py-2 shadow-sm">
                                    ...
                                    <thead class="bg-muted/50">
                                        <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                            ...
                                            <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground cursor-pointer hover:text-foreground" onclick="ordenarTablaMateriales('precio')">
                                                Costo (S/.) <i class="fas fa-sort ml-1"></i>
                                            </th>
    ...
    getMaterialRow: function (material) {
        return `
            <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <td class="p-4 align-middle font-medium text-xs">${material.id}</td>
                <td class="p-4 align-middle">${material.nombre}</td>
                <td class="p-4 align-middle">
                    <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent ${material.tipo === 'ACTIVO' ? 'bg-blue-50 text-blue-700 border-blue-200' : 'bg-gray-100 text-gray-700 border-gray-200'}">
                        ${material.tipo || 'ACTIVO'}
                    </span>
                </td>
                <td class="p-4 align-middle">
                    <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 border-transparent bg-slate-100 text-slate-800">
                        -
                    </span>
                </td>
                <td class="p-4 align-middle">${material.unidadMedida || 'UND'}</td>
                <td class="p-4 align-middle">S/ ${parseFloat(material.costoUnitario || 0).toFixed(2)}</td>
                <td class="p-4 align-middle text-muted-foreground">-</td>
                <td class="p-4 align-middle text-right">
                    <div class="flex justify-end gap-2">
                        <button onclick="window.dashboard.editarMaterial('${material.id}')" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-accent hover:text-accent-foreground h-8 w-8 text-blue-500" title="Editar">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button onclick="window.dashboard.eliminarMaterial('${material.id}')" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-accent hover:text-accent-foreground h-8 w-8 text-red-500" title="Eliminar">
    ...
    getModalMaterialContent: function (data = {}) {
        const isEdit = !!data.id;
        // Reference Replica: Nombre, Tipo (Radio), Unidad, Costo
        return `
            <form id="form-material" onsubmit="event.preventDefault(); window.dashboard.guardarMaterial('${isEdit ? 'edit' : 'create'}', '${data.id || ''}')" class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nombre del Material</label>
                    <input type="text" name="Nombre" value="${data.nombre || ''}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ej. Cable UTP Cat6">
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-2">Tipificación</label>
                    <div class="flex gap-4">
                        <label class="flex-1 flex items-center gap-2 px-4 py-3 rounded-lg border cursor-pointer transition-colors hover:bg-gray-50">
                            <input type="radio" name="Tipo" value="ACTIVO" ${(!data.tipo || data.tipo === 'ACTIVO') ? 'checked' : ''} class="text-blue-600 focus:ring-blue-500">
                            <span class="font-medium">Activo</span>
                        </label>
                        <label class="flex-1 flex items-center gap-2 px-4 py-3 rounded-lg border cursor-pointer transition-colors hover:bg-gray-50">
                            <input type="radio" name="Tipo" value="CONSUMIBLE" ${data.tipo === 'CONSUMIBLE' ? 'checked' : ''} class="text-blue-600 focus:ring-blue-500">
                            <span class="font-medium">Consumible</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <select name="UnidadMedida" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500">
                            <option value="">Seleccionar...</option>
                            <option value="UND" ${data.unidadMedida === 'UND' ? 'selected' : ''}>Unidad (UND)</option>
                            <option value="MTR" ${data.unidadMedida === 'MTR' ? 'selected' : ''}>Metros (MTR)</option>
                            <option value="KG" ${data.unidadMedida === 'KG' ? 'selected' : ''}>Kilogramos (KG)</option>
                            <option value="CJA" ${data.unidadMedida === 'CJA' ? 'selected' : ''}>Caja (CJA)</option>
                        </select>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Costo Unitario (S/.)</label>
                        <input type="number" name="CostoUnitario" value="${data.costoUnitario || ''}" step="0.01" min="0" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" placeholder="0.00">
                    </div>
                </div>
            </form>
        `;
    ...
    getVehiculosTableContent: function () {
        return `
            <div class="w-full space-y-4 p-6">
                <!-- Header Actions -->
                <div class="flex flex-col sm:flex-row justify-between items-center gap-4 mb-6">
                    ...
                    <div class="flex gap-2">
                         <button onclick="window.dashboard.checkDatabaseSetup()" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 border border-input bg-yellow-50 text-yellow-700 hover:bg-yellow-100 h-10 px-4 py-2" title="Reparar Base de Datos">
                            <i class="fas fa-database mr-2"></i> Setup DB
                        </button>
                        <button onclick="window.dashboard.abrirModalVehiculo()" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 bg-primary text-primary-foreground hover:bg-primary/90 h-10 px-4 py-2">
                ...
                <div class="rounded-md border bg-card text-card-foreground shadow-sm overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="w-full text-sm" id="tablaVehiculos">
                            <thead class="bg-muted/50">
                                <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                    <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Placa</th>
                                    <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Marca / Modelo</th>
                                    <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Tipo Activo</th>
                                    <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">Capacidad</th>
    ...
    getVehiculoRow: function (vehiculo) {
        let tipoClass = "bg-blue-50 text-blue-700 border-blue-200";
        if (vehiculo.tipoActivo === "MOTO") tipoClass = "bg-amber-50 text-amber-700 border-amber-200";
        else if (vehiculo.tipoActivo === "MINIVAN") tipoClass = "bg-purple-50 text-purple-700 border-purple-200";

        let estadoClass = vehiculo.estado === "OPERATIVO" ? "text-green-600" : "text-red-600";
        let estadoDot = vehiculo.estado === "OPERATIVO" ? "bg-green-600" : "bg-red-600";

        return `
            <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                <td class="p-4 align-middle font-semibold">${vehiculo.placa}</td>
                <td class="p-4 align-middle text-muted-foreground">${vehiculo.marca}</td>
                <td class="p-4 align-middle">
                    <span class="inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 ${tipoClass}">
                        ${vehiculo.tipoActivo}
                    </span>
                </td>
                <td class="p-4 align-middle text-muted-foreground">${vehiculo.maxVolumen || '-'}</td>
                <td class="p-4 align-middle">
                    <div class="flex items-center gap-2 ${estadoClass} font-medium">
                        <span class="h-2 w-2 rounded-full ${estadoDot}"></span>
                        ${vehiculo.estado}
                    </div>
                </td>
                <td class="p-4 align-middle text-right">
                    <div class="flex justify-end gap-2">
                        <button onclick="window.dashboard.verVehiculo('${vehiculo.placa}')" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-accent hover:text-accent-foreground h-8 w-8 text-blue-600" title="Ver Detalles">
                            <i class="fas fa-eye"></i>
                        </button>
                        <button onclick="window.dashboard.editarVehiculo('${vehiculo.placa}')" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-accent hover:text-accent-foreground h-8 w-8 text-orange-500" title="Editar">
                            <i class="fas fa-edit"></i>
                        </button>
                        <button onclick="window.dashboard.eliminarVehiculo('${vehiculo.placa}')" class="inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 hover:bg-accent hover:text-accent-foreground h-8 w-8 text-red-500" title="Eliminar">
                            <i class="fas fa-trash"></i>
                        </button>
                    </div>
                </td>
    ...
    getModalVehiculoContent: function (data = {}) {
        const isEdit = !!data.placa;
        return `
            <form id="form-vehiculo" onsubmit="event.preventDefault(); window.dashboard.guardarVehiculo('${isEdit ? 'edit' : 'create'}', '${data.placa || ''}')" class="space-y-4">
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Placa</label>
                        <input type="text" name="Placa" value="${data.placa || ''}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm uppercase" placeholder="ABC-123">
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Marca / Modelo</label>
                        <input type="text" name="Marca" value="${data.marca || ''}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" placeholder="Toyota Hilux">
                    </div>
                </div>

                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Tipo de Activo</label>
                        <select name="TipoActivo" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" onchange="window.dashboard.handleTipoActivoChange(this)">
                            <option value="CAMIONETA" ${data.tipoActivo === 'CAMIONETA' ? 'selected' : ''}>Camioneta</option>
                            <option value="MINIVAN" ${data.tipoActivo === 'MINIVAN' ? 'selected' : ''}>Minivan</option>
                            <option value="MOTO" ${data.tipoActivo === 'MOTO' ? 'selected' : ''}>Moto</option>
                        </select>
                    </div>
                    <div class="space-y-2">
                        <label class="text-sm font-medium leading-none">Estado</label>
                        <select name="Estado" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                            <option value="OPERATIVO" ${(!data.estado || data.estado === 'OPERATIVO') ? 'selected' : ''}>Operativo</option>
                            <option value="TALLER" ${data.estado === 'TALLER' ? 'selected' : ''}>Taller / Mantenimiento</option>
                        </select>
                    </div>
                </div>

                <div class="space-y-2 p-4 bg-muted/50 rounded-lg">
                    <label class="text-sm font-medium leading-none">Capacidad de Carga</label>
                    <select name="MaxVolumen" id="vehiculo-capacidad" class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                        <option value="ALTO" ${data.maxVolumen === 'ALTO' ? 'selected' : ''}>Alto Volumen</option>
                        <option value="BAJO" ${data.maxVolumen === 'BAJO' ? 'selected' : ''}>Bajo Volumen</option>
                    </select>
                    <p class="text-xs text-muted-foreground hidden" id="moto-info">
                        <i class="fas fa-info-circle mr-1"></i> Moto asignada automáticamente a Bajo Vol.
                    </p>
                </div>
            </form>
        `;
    ...
    // --- Gestión de Perfiles (Roles) Adapter & Grid ---

    getRoleMetadata: function (roleName) {
        // Adapter to match Reference Aesthetics without DB columns
        ...
        if (lowerName.includes('admin') || lowerName.includes('gerencia')) {
            metadata.icon = '<i class="fas fa-shield-alt"></i>';
            metadata.color = 'bg-rose-500';
        } else if (lowerName.includes('supervisor') || lowerName.includes('jefe')) {
            metadata.icon = '<i class="fas fa-user-tie"></i>';
            metadata.color = 'bg-blue-500';
        } else if (lowerName.includes('técnico') || lowerName.includes('operario')) {
            metadata.icon = '<i class="fas fa-hard-hat"></i>';
            metadata.color = 'bg-amber-500';
        } else if (lowerName.includes('logística') || lowerName.includes('almacén')) {
            metadata.icon = '<i class="fas fa-box-open"></i>';
            metadata.color = 'bg-emerald-500';
        } else if (lowerName.includes('rrhh') || lowerName.includes('humanos')) {
            metadata.icon = '<i class="fas fa-users"></i>';
            metadata.color = 'bg-purple-500';
        }

        return metadata;
    },

    getRolesGridContent: function (roles = []) {
        if (!roles || roles.length === 0) {
            return `
                <div class="text-center py-12 bg-white rounded-lg border border-dashed border-gray-300">
                    ...
                    <p class="text-sm text-gray-500 mt-1">Comienza creando un nuevo rol para tu organización.</p>
                </div>
            `;
        }

        const cardsHtml = roles.map(role => {
            const meta = this.getRoleMetadata(role.name);
            return `
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-6 relative group">
                <div class="flex items-start justify-between mb-4">
                    <div class="w-12 h-12 rounded-lg ${meta.color} ${meta.textColor} flex items-center justify-center text-xl shadow-sm">
                        ${meta.icon}
                    </div>
                    <span class="inline-flex items-center rounded-full border border-gray-200 bg-white px-2.5 py-0.5 text-xs font-semibold text-gray-700 transition w-fit">
                        ${role.userCount || 0} usuarios
                    </span>
                </div>

                <h3 class="text-lg font-bold text-gray-900 mb-1">${role.name}</h3>
                <p class="text-sm text-gray-500 mb-6 line-clamp-2 min-h-[40px]">${role.description || 'Sin descripción'}</p>

                <div class="flex gap-3 pt-4 border-t border-gray-100">
                    <button class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors" title="Asignar Usuarios">
                        <i class="fas fa-users text-gray-400"></i> Asignar
                    </button>
                    <button onclick="window.dashboard.editarRol(${role.id})" class="flex-1 inline-flex items-center justify-center gap-2 px-3 py-2 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors" title="Configurar Rol">
                        <i class="fas fa-cog text-gray-400"></i> Configurar
                    </button>
                </div>
            </div>`;
        }).join('');
    ...
    getModalRolContent: function (data = {}) {
        const isEdit = !!data.id;
        return `
            <form id="form-role" onsubmit="event.preventDefault(); window.dashboard.guardarRol('${isEdit ? 'edit' : 'create'}', '${data.id || ''}')" class="space-y-6">
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Nombre del Rol</label>
                    <input type="text" name="Name" value="${data.name || ''}" required class="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500" placeholder="Ej. Supervisor de Campo">
                    <p class="text-xs text-muted-foreground mt-1">El nombre debe ser único en el sistema.</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1">Descripción</label>
                    <textarea name="Description" rows="3" class="flex w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 resize-none" placeholder="Describe las responsabilidades de este rol...">${data.description || ''}</textarea>
                </div>
            </form>
        `;
    }
